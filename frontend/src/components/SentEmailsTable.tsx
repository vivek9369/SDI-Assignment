...
import { apiClient, SentEmail } from '@/api.ts';

interface SentEmailsTableProps {
  refresh: number;
}

export const SentEmailsTable: React.FC<SentEmailsTableProps> = ({ refresh }) => {
  ...
  const fetchEmails = async () => {
    try {
      setLoading(true);
      const response = await apiClient.getSentEmails(page, 20);
    ...
    return (
      <Card>
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">Scheduled emails will appear here after they are sent</p>
        </div>
      </Card>
    );
  }
          ...
          <table className="w-full">
            <thead className="border-b border-gray-200 bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-sm font-semibold text-gray-900">Sent Time</th>
